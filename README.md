...
3. Затем, после установки всех пакетов:
  ...
  - `$ npm start` — чтобы собрать проект и запустить его на локальном сервере. После запуска в браузере должна автоматически открыться страница с собранным проектом (http://localhost:5300).